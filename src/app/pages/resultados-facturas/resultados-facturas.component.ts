...
export class ResultadosFacturasComponent implements OnInit {
  ...
  constructor(private router: Router, private route: ActivatedRoute, config: NgbModalConfig, private modalService: NgbModal,
    ...
    config.size = 'lg';
    this.route.queryParams.subscribe(params => this.clienteName = params['numeroId']);
  }
  ...
  selectall() {
    this.polizas.map(poliza => poliza.detalle = poliza.detalle.map(data => { if (data.permite) { data.selected = !data.selected; } return data; }));
  }
  ...
  pagarCon(metodo, modal = null) {
    switch (metodo) {
      ...
      case 'kushki':
        ...
        break;
      case 'bancoomeva':
        this.service.presentToast("¡Atención!", "No podemos comunicarnos con la pasarela en estos momentos", "warning");
        break;
    }
